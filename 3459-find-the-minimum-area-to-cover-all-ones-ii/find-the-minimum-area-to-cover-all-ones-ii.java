class Solution {
    ...
    private int solve(int[][] grid) {
        ...
        for (int i = 0; i + 1 < n; i++)
            for (int j = 0; j + 1 < m; j++) {
                res = Math.min(res, minimumSum2(grid, 0, i, 0, m - 1) + minimumSum2(grid, i + 1, n - 1, 0, j)
                        + minimumSum2(grid, i + 1, n - 1, j + 1, m - 1));
                res = Math.min(res, minimumSum2(grid, 0, i, 0, j) + minimumSum2(grid, 0, i, j + 1, m - 1)
                        + minimumSum2(grid, i + 1, n - 1, 0, m - 1));
            }
        for (int i = 0; i + 2 < n; i++)
            for (int j = i + 1; j + 1 < n; j++)
                res = Math.min(res, minimumSum2(grid, 0, i, 0, m - 1) + minimumSum2(grid, i + 1, j, 0, m - 1)
